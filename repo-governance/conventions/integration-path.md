...
- Reconcile local `main` after every merge. The pull request lands from a `worktrees/` checkout, so `origin/main` moves and the primary checkout's `main` does not. Git reports no error; that checkout is simply behind. It is also where releases are cut, so leaving it stale means the next cut reads a `main` that is not `main`. Run `git fetch origin`, then `git merge --ff-only origin/main` there, and prove it with `git rev-list --left-right --count HEAD...origin/main` reading `0 0`. Where a clone has no primary checkout — verify with `git worktree list`, never with `git rev-parse --is-bare-repository`, which answers for the checkout it runs in — `git fetch origin main:main` does the same job. No hook can enforce this: it runs after the merge, in a checkout the merge never touched.
