...
- `pre-commit` runs `lint-staged`, which formats staged files only.
- `pre-push` runs `cargo xtask test-quick` under the pinned HIPPO guard. Exit `75` means the host was busy: retry that same invocation once the condition clears, never bypass it.
